...
import { Camera, Calendar, ArrowLeft, Heart, Loader2, Sparkles } from 'lucide-react';
...
export default function AddMemory() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user || !db || !storage || !image) {
      if (!db || !storage) alert('Firebase is not configured. Please check your environment variables.');
    ...
    try {
      const storageRef = ref(storage, `memories/${user.uid}/${Date.now()}-${image.name}`);
  ...
  return (
    <div className="min-h-screen bg-background p-6 md:p-12 lg:p-24">
      <div className="max-w-2xl mx-auto">
        <Link href={`/m/${mementoId}`} className="inline-flex items-center text-muted hover:text-foreground mb-12 transition-colors group">
        ...
        <div className="mb-12 space-y-2">
          <div className="flex items-center space-x-2 text-primary mb-2">
            ...
            <span className="text-xs font-bold uppercase tracking-[0.2em]">Add a Moment</span>
          </div>
          <h1 className="text-4xl md:text-5xl font-serif font-medium">Capture a memory</h1>
          <p className="text-muted font-light text-lg">What makes this moment worth holding onto?</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
              ...
              {imagePreview ? (
                <>
                  ...
                  <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <p className="text-white font-medium px-6 py-2 border border-white/30 rounded-full backdrop-blur-md">Change photo</p>
                  </div>
                </>
              ) : (
                <div className="text-center p-12 space-y-4">
                  ...
                  <div>
                    <p className="text-xl font-serif font-medium">Add a photo</p>
                    <p className="text-sm text-muted/60 mt-1">The heart of this memory</p>
                  </div>
                </div>
              )}
              <input
          ...
          <div className="bg-white/50 p-8 md:p-12 rounded-[2.5rem] border border-border/50 shadow-sm space-y-10">
            <div className="space-y-4">
              <label className="text-xs font-bold text-muted uppercase tracking-[0.2em]">The Story</label>
              <textarea
                value={description}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-4">
                <label className="text-xs font-bold text-muted uppercase tracking-[0.2em]">Title (Optional)</label>
                <input
                  type="text"
                  value={title}
